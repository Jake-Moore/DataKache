...
internal class MongoListener(private val service: MongoDatabaseService) : ClusterListener, ServerMonitorListener {
    ...
    private val serverPingMap: Cache<ServerAddress, Long> = CacheBuilder
        .newBuilder()
    ...
    override fun clusterDescriptionChanged(event: ClusterDescriptionChangedEvent) {
        ...
        if (event.newDescription.serverDescriptions.isNotEmpty()) {
            updateAverageServerPing(event.newDescription.serverDescriptions)
        }
    }
    ...
    override fun serverHeartbeatSucceeded(event: ServerHeartbeatSucceededEvent) {
        ...
        if (client != null) {
            ...
            updateAverageServerPing(client.getClusterDescription().serverDescriptions)
        }
    }

    private fun updateAverageServerPing(servers: List<ServerDescription>) {
        if (service.activeListener != listenerID) return // This listener is not the active one, ignore the event

        for (server in servers) {
            serverPingMap.put(server.address, server.roundTripTimeNanos)
        }
        recalculateAveragePing()
    }

    private var lastNotifiedPingEpochMS = 0L
    private fun recalculateAveragePing() {
        ...
        var pingSumNanos = 0L
        for (ping in serverPingMap.asMap().values) {
            pingSumNanos += ping
        }

        // Set average ping nanos
        this.service.averagePingNanos = if (serverPingMap.size() > 0) {
            pingSumNanos / serverPingMap.size()
        } else {
            -1L // No servers available, set to -1 to indicate no ping
        }

        // debug the average ping at most every 1 second
        val now = System.currentTimeMillis()
        if (now - lastNotifiedPingEpochMS > 1000) {
            service.debug(
                "MongoDB average ping: ${service.averagePingNanos / 1_000_000} ms " +
                    "(${service.averagePingNanos} ns)"
            )
            lastNotifiedPingEpochMS = now
        }
    }
}
